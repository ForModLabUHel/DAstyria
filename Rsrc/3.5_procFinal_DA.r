...
data.all$siteType2015 <- 3
data.all$siteType2018 <- 3
data.all$siteType2021 <- 3
...
if(FALSE){
  ...
  #   # sampleX[,id:=climID]
  # }
  nSamples <- ceiling(dim(data.all)[1]/maxSitesRun)
  set.seed(1)
  sampleset <- sample(1:nSamples, nrow(data.all),  replace=T)
  samples <- split(data.all, sampleset)
}



# adding sampleID, sampleRow (= row within sample)
# data.all[,sampleID:=sampleset]
# data.all[,sampleRow:=1:length(h2015),by=sampleID]
...
#    running further scripts in multiple sections. Number of split parts is defined in splitRangeFinal variable (in settings).
...
#   split_length <- ceiling(nrow(data.all)/length(splitRangeFinal))
#   data.all <- data.all[, split_id := NA]
#   
#   
#   data.all$split_id[1:split_length] <- 1
#   for (i in 2:(max(splitRangeFinal)-1)) {
#     data.all$split_id[((i-1)*split_length+1):(split_length*i)] <- i
#   }
#   data.all$split_id[((length(splitRangeFinal)-1)*split_length+1):(nrow(data.all))] <- length(splitRangeFinal)
#   
#   # Split the table to list of elements. Splitting is done based on the split_id.
#   split_list <- split(data.all,data.all$split_id)

# split_list[[1]] <- split_list[[1]][1:3000]
# split_list[[2]] <- split_list[[2]][1:3000]
# split_list[[3]] <- split_list[[3]][1:3000]
# split_list[[4]] <- split_list[[4]][1:3000]
# 
siteTypeX = 3
yearX=yearRast
...
writeRaster(SVI,filename = paste0("finalRast/SVI_prebas_",yearX,".tif"))
writeRaster(WblgPreb,filename = paste0("finalRast/Wbl_prebas_",yearX,".tif"))
writeRaster(WabgPreb,filename = paste0("finalRast/Wabg_prebas_",yearX,".tif"))
writeRaster(Vpreb,filename = paste0("finalRast/V_prebas_",yearX,".tif"))
